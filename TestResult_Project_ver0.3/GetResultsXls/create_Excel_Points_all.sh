...
function check_result()
{
  ...
  if [ $? -eq 0 ];
  then
    echo "TestType:[${TestType}] Platform:[${Platform}] TestCase:[${TestCase}] setting result Excel file success!" >> $okfile
  else
    echo "TestType:[${TestType}] Platform:[${Platform}] TestCase:[${TestCase}] setting result Excel file failed!" >> $errfile
  fi
